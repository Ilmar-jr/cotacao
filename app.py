...
def campo(label, componente, largura):
    # Alterado de width=largura para xs=12 e md=largura para quebrar no mobile
    return dbc.Col([
...
def layout_login():
    ...
    ], fluid=True, className="px-3 px-md-5")
...
def layout_cotacao_interna():
                    ...
                    dbc.Card([
                        dbc.CardBody([
                            ...
                            dash_table.DataTable(
                                ...
                                style_table={'overflowX': 'auto'}, # Ajuste para mobile
            ...
            dbc.Tab(label="📊 Relatório & Análise de Negociação", tab_id="tab-relatorio", children=[
                html.Div([
                    ...
                    dbc.Row([
                        ...
                        ])], className="card-cotacao"), xs=12, sm=6, lg=3, className="mb-3 mb-lg-0"),
                        ...
                        ])], className="card-cotacao"), xs=12, sm=6, lg=3),
                    ], className="mb-4"),
    ...
    ], style={'minHeight': '100vh'}, className="px-3 px-md-5 pb-5 pt-3", fluid=True)
...
def montar_tabela_cotacoes(busca=None, status=None, data_inicio=None, data_fim=None):
    ...
    for c in cotacoes:
        ...
        try:
            ...
            data_corrigida = str(c['data_criacao'])[:16]
...
def layout_responder_fornecedor(token):
    ...
    if cotacao is None:
        return dbc.Container([html.Div([html.I(className="bi bi-exclamation-triangle me-2", style={'color': COR_PERIGO}), html.Span("Link inválido ou cotação não encontrada.", style={'color': COR_TEXTO, 'fontSize': '18px'})], style={'marginTop': '60px'})], className="px-3 px-md-5", fluid=True)

    rodada = cotacao['rodada_atual']
    ja_respondida = cotacao['status'] == 'respondida'
    
    cabecalho = dbc.Row([
        dbc.Col("Grupo", xs=3, md=2), 
        dbc.Col("Item", xs=3, md=3), 
        dbc.Col("Marcas Aceitas", xs=2, md=2), 
        dbc.Col("Unid", xs=1, md=1), 
        dbc.Col("Qtd", xs=1, md=1), 
        dbc.Col(f"Preço ({rodada}ª Rod.)", xs=2, md=3)
    ...
    for item in cotacao['itens']:
        ...
        linhas.append(dbc.Row([
            dbc.Col(item['grupo'] or "-", xs=3, md=2, style={'color': COR_TEXTO, 'fontSize': '13px'}), 
            dbc.Col(item['item'], xs=3, md=3, style={'color': COR_TEXTO, 'fontSize': '13px'}), 
            dbc.Col(item.get('marca', '-'), xs=2, md=2, style={'color': COR_ACCENT_2, 'fontSize': '13px'}), 
            dbc.Col(item['volume'] or "-", xs=1, md=1, style={'color': COR_TEXTO, 'fontSize': '13px'}), 
            dbc.Col(item['qtd'], xs=1, md=1, style={'color': COR_TEXTO, 'fontSize': '13px'}), 
            dbc.Col(campo_preco, xs=2, md=3)
    ...
    return dbc.Container([
        ...
        dbc.Card([
            dbc.CardBody([
                ...
                html.Div([cabecalho, html.Div(linhas)], style={'overflowX': 'auto', 'minWidth': '600px'}),
...
def layout_resultado_cotacao(cotacao_id):
    ...
    if cotacao_completa is None:
        return dbc.Container([html.Div([html.I(className="bi bi-exclamation-triangle me-2", style={'color': COR_PERIGO}), html.Span("Cotação não encontrada.", style={'color': COR_TEXTO, 'fontSize': '18px'})], style={'marginTop': '60px', 'marginBottom': '20px'}), dcc.Link("← Voltar", href="/", style={'color': COR_ACCENT_2})], className="px-3 px-md-5", fluid=True)

    respondida = cotacao_completa['status'] == 'respondida'
    linhas_tabela = []
    rodadas_disponiveis = set()
    
    # Fuso blindado para o cabeçalho do Histórico
    try:
        dt_obj = pd.to_datetime(cotacao_completa['data_criacao'])
        if dt_obj.tz is None: dt_obj = dt_obj.tz_localize('UTC')
        data_cabecalho = dt_obj.tz_convert('America/Sao_Paulo').strftime('%d/%m/%Y %H:%M')
    except:
        data_cabecalho = str(cotacao_completa['data_criacao'])[:16]
    ...
    return dbc.Container([
        ...
        html.Div([
            ...
            html.Div([html.Span(f"Fornecedor: {cotacao_completa['fornecedor'] or '-'}   •   Criada em {data_cabecalho}   •   ", style={'color': COR_MUTED, 'fontSize': '14px'}), dbc.Badge(cotacao_completa['status'].upper(), color="success" if respondida else "warning")], style={'marginTop': '4px'}),
        ], style={'padding': '18px 4px 8px 4px'}),
        ...
        dbc.Card([
            dbc.CardBody([
                dbc.Row([
                    ...
                    ], xs=12, md=6, className="mb-3 mb-md-0", style={'borderRight': f'1px solid {COR_BORDA}'}),
                    
                    dbc.Col([
                        html.Div("Exportar para Excel:", style={'color': COR_TEXTO, 'fontSize': '14px', 'marginBottom': '10px', 'fontWeight': '600'}),
                        dbc.Row([
                            dbc.Col(dcc.Dropdown(id='dropdown-rodada-excel', options=opcoes_rodadas, placeholder="Escolha a rodada...", style={'color': '#000'}), xs=12, lg=7),
                            dbc.Col(dbc.Button([html.I(className="bi bi-file-earmark-excel me-2"), "Baixar"], id='btn-baixar-excel', n_clicks=0, style={'backgroundColor': COR_SUCESSO, 'border': 'none', 'fontWeight': '700', 'width': '100%'}, size="sm"), xs=12, lg=5)
                        ], className="g-2"),
...
@callback(
    ...
    Output('tabela-matriz-comparativa', 'children'), # Saída nova
...
def atualizar_painel_relatorio(dt_inicio, dt_fim, fornecedor, produto):
    ...
    if not dados:
        ...
        return "0", "R$ 0,00", "R$ 0,00", "R$ 0,00", {'color': COR_MUTED}, fig_vazia, fig_vazia, msg_vazia, msg_vazia
        ...
        if valores:
            ...
            for c in colunas_fornecedores:
                if row[c] == menor_valor:
                    # Aplica cor verde na célula com menor preço
    ...
    # ============================================================
    # FUSO HORÁRIO BLINDADO (Tabela Detalhada)
    # ============================================================
    ...
    tabela_detalhada = dash_table.DataTable(
        ...
        style_table={'overflowX': 'auto'}, # Ajuste mobile
...
def finalizar_cotacao(n_clicks, fornecedor_sel, lista_atual):
    ...
    resultado = html.Div([
        ...
        dbc.Row([
            dbc.Col(html.Div(link, className="caixa-link"), xs=10, md=11),
