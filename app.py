...
app = Dash(
    ...
    # TAG OBRIGATÓRIA PARA RESPONSIVIDADE NO MOBILE:
...
# Para produção no Render, utilize variáveis de ambiente.
...
def campo(label, componente, largura):
    """Função adaptada para ser responsiva: 12 colunas no telemóvel, 'largura' no PC"""
    return dbc.Col([
    ...
    return dbc.Container([
        ...
        dbc.Tabs([
            ...
            dbc.Tab(label="📋 Gerenciar & Criar Cotações", tab_id="tab-operacional", children=[
                html.Div([
                    # STORE NECESSÁRIO PARA SALVAR ITENS NA MEMÓRIA
                    ...
                    dbc.Card([
                        dbc.CardBody([
                            ...
                            dash_table.DataTable(
                                ...
                                style_table={'overflowX': 'auto'}, # RESPONSIVIDADE TABELA
...
def montar_tabela_cotacoes(busca=None, status=None, data_inicio=None, data_fim=None):
    ...
    linhas = []
    for c in cotacoes:
        # CONVERSÃO DE FUSO HORÁRIO AQUI TAMBÉM:
        try:
            data_corrigida = pd.to_datetime(c['data_criacao']).tz_convert('America/Sao_Paulo').strftime('%d/%m/%Y %H:%M')
        except:
            data_corrigida = c['data_criacao'] # Fallback caso haja falha
...
def layout_responder_fornecedor(token):
    ...
    if cotacao is None:
        return dbc.Container([html.Div([html.I(className="bi bi-exclamation-triangle me-2", style={'color': COR_PERIGO}), html.Span("Link inválido ou cotação não encontrada.", style={'color': COR_TEXTO, 'fontSize': '18px'})], style={'marginTop': '60px'})], fluid=True, className="p-3 p-md-5")

    rodada = cotacao['rodada_atual']
    ja_respondida = cotacao['status'] == 'respondida'
    
    cabecalho = dbc.Row([
        dbc.Col("Grupo", xs=3, md=3), 
        dbc.Col("Item", xs=3, md=3), 
        dbc.Col("Vol", xs=2, md=2), 
        dbc.Col("Qtd", xs=1, md=1), 
        dbc.Col(f"Preço ({rodada}ª Rod.)", xs=3, md=3)
    ...
    for item in cotacao['itens']:
        ...
        linhas.append(dbc.Row([
            dbc.Col(item['grupo'] or "-", xs=3, md=3, style={'color': COR_TEXTO, 'fontSize': '13px'}),
            dbc.Col(item['item'], xs=3, md=3, style={'color': COR_TEXTO, 'fontSize': '13px'}),
            dbc.Col(item['volume'] or "-", xs=2, md=2, style={'color': COR_TEXTO, 'fontSize': '13px'}),
            dbc.Col(item['qtd'], xs=1, md=1, style={'color': COR_TEXTO, 'fontSize': '13px'}),
            dbc.Col(campo_preco, xs=3, md=3)
        ], className="py-2 align-items-center", style={'borderBottom': f'1px solid {COR_BORDA}'}))

    return dbc.Container([
        html.Div([
            html.Div([html.I(className="bi bi-send-check-fill me-2", style={'color': COR_ACCENT_2}), html.Span(f"NEGOCIAÇÃO ONLINE - {rodada}ª RODADA", className="titulo-cotacao", style={'fontSize': '22px', 'fontWeight': '700', 'color': COR_TEXTO})]),
        ...
        dbc.Card([
            dbc.CardBody([
                ...
                # Container com scroll lateral para garantir que não quebra no telemóvel
                html.Div([
                    cabecalho, 
                    html.Div(linhas)
                ...
                html.Div([
                    dbc.Button([html.I(className="bi bi-check2-circle me-2"), "Enviar preços da Rodada"], id='Btn-enviar-precos', n_clicks=0, style={'backgroundColor': COR_ACCENT, 'border': 'none', 'color': '#12161c', 'fontWeight': '700', 'marginTop': '18px', 'width': '100%'}, disabled=ja_respondida),
...
# ============================================================
# LAYOUT 3: TELA INTERNA -- HISTÓRICO DA COTAÇÃO
# ============================================================
...
def layout_resultado_cotacao(cotacao_id):
    ...
    if cotacao_completa is None:
        return dbc.Container([html.Div([html.I(className="bi bi-exclamation-triangle me-2", style={'color': COR_PERIGO}), html.Span("Cotação não encontrada.", style={'color': COR_TEXTO, 'fontSize': '18px'})], style={'marginTop': '60px', 'marginBottom': '20px'}), dcc.Link("← Voltar", href="/", style={'color': COR_ACCENT_2})], fluid=True, className="p-3 p-md-5")
    ...
    return dbc.Container([
        ...
        html.Div([
            ...
            html.Div([html.I(className="bi bi-bar-chart-line-fill me-2", style={'color': COR_ACCENT_2}), html.Span(f"HISTÓRICO - COTAÇÃO Nº {cotacao_completa['id']}", className="titulo-cotacao", style={'fontSize': '22px', 'fontWeight': '700', 'color': COR_TEXTO})], style={'marginTop': '10px'}),
            html.Div([html.Span(f"Fornecedor: {cotacao_completa['fornecedor'] or '-'} • Criada em {cotacao_completa['data_criacao'][:10]} • ", style={'color': COR_MUTED, 'fontSize': '14px'}), dbc.Badge(cotacao_completa['status'].upper(), color="success" if respondida else "warning")], style={'marginTop': '4px'}),
        ], style={'padding': '18px 4px 8px 4px'}),
        
        # CARD DE GERENCIAMENTO RESPONSIVO
        dbc.Card([
            dbc.CardBody([
                dbc.Row([
                    dbc.Col([
                        html.Div("Negociar valores:", style={'color': COR_TEXTO, 'fontSize': '14px', 'marginBottom': '10px', 'fontWeight': '600'}),
                        dbc.Button([html.I(className="bi bi-arrow-repeat me-2"), "Reabrir Cotação (Nova Rodada)"], id='btn-nova-rodada', n_clicks=0, style={'backgroundColor': COR_ACCENT, 'border': 'none', 'color': '#12161c', 'fontWeight': '700', 'width': '100%'}, size="sm"),
                        html.Div(id='feedback-nova-rodada', style={'marginTop': '10px'})
                    ], xs=12, md=6, className="border-md-right mb-3 mb-md-0", style={'borderRight': f'1px solid {COR_BORDA}'}),
                    
                    dbc.Col([
                        html.Div("Exportar para Excel:", style={'color': COR_TEXTO, 'fontSize': '14px', 'marginBottom': '10px', 'fontWeight': '600'}),
                        dbc.Row([
                            dbc.Col(dcc.Dropdown(id='dropdown-rodada-excel', options=opcoes_rodadas, placeholder="Escolha a rodada...", style={'color': '#000'}), xs=12, sm=7, className="mb-2 mb-sm-0"),
                            dbc.Col(dbc.Button([html.I(className="bi bi-file-earmark-excel me-2"), "Baixar"], id='btn-baixar-excel', n_clicks=0, style={'backgroundColor': COR_SUCESSO, 'border': 'none', 'fontWeight': '700', 'width': '100%'}, size="sm"), xs=12, sm=5)
                        ]),
                        html.Div(id='feedback-excel', style={'marginTop': '10px'})
                    ], xs=12, md=6, className="ps-md-4")
                ])
            ])
...
@callback(
    Output('produtos', 'options'), Output('produtos', 'value'),
    Output('input-volume', 'value'), Output('input-qtd', 'value'),
    Output('lista-store', 'data'), Output('tabela-lista', 'data'),
    Output('badge-total-itens', 'children'), Output('badge-total-qtd', 'children'),
    Input('grupo', 'value'), Input('Btn-salvar', 'n_clicks'), Input('Btn-remover', 'n_clicks'),
    State('tab-pn', 'value'), State('grupo', 'value'), State('produtos', 'value'),
    State('input-volume', 'value'), State('input-qtd', 'value'),
    State('lista-store', 'data'), State('tabela-lista', 'selected_rows'),
    prevent_initial_call=True
)
def gerenciar(grupo_selecionado, salvar, remover, fornecedor_sel, grupo_sel, produto_sel, volume, qtd, lista_atual, lines_selecionadas):
    def resumo(lista):
    ...
    if botao_acionado == 'Btn-remover':
        if not lines_selecionadas:
            return no_update, no_update, no_update, no_update, no_update, no_update, no_update, no_update
        lista_atual.pop(lines_selecionadas[0])
        itens_txt, qtd_txt = resumo(lista_atual)
...
def finalizar_cotacao(n_clicks, fornecedor_sel, lista_atual):
    if not lista_atual:
        return dbc.Alert("Adicione ao menos um item antes de finalizar.", color="warning", style={'marginTop': '10px'}), no_update, no_update
    ...
    resultado = html.Div([
        ...
        dbc.Row([
            dbc.Col(html.Div(link, className="caixa-link"), xs=10, md=9),
            dbc.Col(dcc.Clipboard(target_id=None, content=link, style={'fontSize': '22px', 'color': COR_ACCENT_2, 'cursor': 'pointer'}), xs=2, md=1, className="d-flex align-items-center justify-content-center"),
...
@callback(
    Output('url', 'pathname'), Input('tabela-cotacoes', 'active_cell'), State('tabela-cotacoes', 'data'),
    prevent_initial_call=True
)
...
@callback(
    ...
    Input('url', 'pathname'), Input('Btn-finalizar', 'n_clicks'),
    Input('filtro-busca', 'value'), Input('filtro-status', 'value'),
    Input('filtro-data-inicio', 'date'), Input('filtro-data-fim', 'date'),
...
@callback(
    ...
    Input('Btn-enviar-precos', 'n_clicks'), State({'type': 'preco-input', 'index': ALL}, 'value'),
    State({'type': 'preco-input', 'index': ALL}, 'id'), State('token-cotacao', 'data'), State('rodada-atual-fornecedor', 'data'),
    prevent_initial_call=True
)
def enviar_precos(n_clicks, valores, ids, token, rodada_atual):
    ...
    if invalidos:
        return dbc.Alert("Preencha o preço de todos os itens com um número válido (ex: 12,50) antes de enviar.", color="warning"), False
...
@callback(
    ...
    Input('btn-nova-rodada', 'n_clicks'), State('url', 'pathname'),
    prevent_initial_call=True
)
def disparar_nova_rodada(n_clicks, pathname):
    if not pathname or not pathname.startswith('/resultado/'): return no_update
    cotacao_id = pathname.replace('/resultado/', '').strip('/')
    bd.abrir_nova_rodada_negociacao(int(cotacao_id))
    return dbc.Alert("Nova rodada aberta com sucesso! O mesmo link do fornecedor foi liberado para os novos preços.", color="success")


@callback(
    Output("download-excel", "data"), Output("feedback-excel", "children"),
    Input("btn-baixar-excel", "n_clicks"), State("dropdown-rodada-excel", "value"), State("url", "pathname"),
    prevent_initial_call=True
)
def gerar_excel_rodada(n_clicks, rodada_selecionada, pathname):
    ...
    if not pathname or not pathname.startswith('/resultado/'): return no_update, no_update
    
    cotacao_id = pathname.replace('/resultado/', '').strip('/')
    cotacao_completa = bd.buscar_todas_rodadas_id(int(cotacao_id))
    if not cotacao_completa: return no_update, no_update
    ...
    if not itens_filtrados:
        return no_update, dbc.Alert("Nenhum dado encontrado.", color="danger", style={'fontSize': '12px', 'padding': '5px'})
    ...
    for item in itens_filtrados:
        ...
        dados_excel.append({
            "Fornecedor": fornecedor_nome, "Produto": item['item'], "Unidade": item['volume'] or "-",
            "Quantidade": qtd, "Valor Total": valor_total
        })
        
    df = pd.DataFrame(dados_excel)
    nome_arquivo = f"Cotacao_{cotacao_id}_Rodada_{rodada_selecionada}.xlsx"
...
@callback(
    ...
    Output('grafico-economia-fornecedor', 'figure'), Output('tabela-relatorio-detalhada', 'children'),
    Input('rel-data-inicio', 'date'), Input('rel-data-fim', 'date'),
    Input('rel-fornecedor', 'value'), Input('rel-produto', 'value'),
    prevent_initial_call=False
)
def atualizar_painel_relatorio(dt_inicio, dt_fim, fornecedor, produto):
    ...
    fig_evolucao = px.line(df_evolucao, x='rodada_label', y='total_item', color='cotacao_id', markers=True, labels={'total_item': 'Valor Total (R$)', 'rodada_label': 'Fase de Negociação', 'cotacao_id': 'Cotação Nº'})
    ...
    fig_economia = px.bar(df_econ, x='fornecedor', y='economia', color='economia', color_continuous_scale=['red', 'yellow', 'green'], labels={'economia': 'Economia (R$)', 'fornecedor': 'Fornecedor'})
    ...
    # 4. Tabela Detalhada com CORREÇÃO DE FUSO HORÁRIO
    df['data_criacao'] = pd.to_datetime(df['data_criacao'], utc=True).dt.tz_convert('America/Sao_Paulo').dt.strftime('%d/%m/%Y %H:%M')
    df['preco_unitario'] = df['preco_unitario'].apply(lambda x: f"R$ {x:,.2f}")
    df['total_item'] = df['total_item'].apply(lambda x: f"R$ {x:,.2f}")
    
    tabela = dash_table.DataTable(
        columns=[
            {"name": "Cotação Nº", "id": "cotacao_id"}, {"name": "Data", "id": "data_criacao"},
            {"name": "Fornecedor", "id": "fornecedor"}, {"name": "Rodada", "id": "rodada"},
            {"name": "Produto", "id": "produto"}, {"name": "Qtd", "id": "qtd"},
            {"name": "Preço Unit.", "id": "preco_unitario"}, {"name": "Subtotal", "id": "total_item"},
        ],
    ...
    return str(cotacoes_unicas), f"R$ {val_inicial:,.2f}", f"R$ {val_final:,.2f}", texto_resultado, estilo_resultado, fig_evolucao, fig_economia, tabela
